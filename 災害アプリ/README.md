...
> 🌐 **公開中: https://kota0004.github.io/test02/**
> 毎時の雨量取り込みがそのまま反映されます。位置が人手で未確認の地点が22件含まれるため、
> 画面上でも区別して表示しています。参考情報であり、通行の可否を保証するものではありません。
...
1. **未確認22件＋要確認6件の位置確認** — `python3 tools/review_spots.py --spots data/spots_chiba.json --ipad`
   - レビュー画面の **「要確認」** フィルタに、点検で見つかった6件が出ます
     （座標が重なっている2組・高低差が不自然な2件）。理由も一緒に表示されます
